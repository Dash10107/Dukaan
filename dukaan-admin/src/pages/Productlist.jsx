...
import { getProducts, createProducts, resetState, bulkUploadProducts } from "../features/product/productSlice"
import { getBrands } from "../features/brand/brandSlice"
import { getCategories } from "../features/pcategory/pcategorySlice"
import { getColors } from "../features/color/colorSlice"
import { uploadImg, delImg } from "../features/upload/uploadSlice"
import { Heart, Edit, Trash2, Plus, X, Upload } from "lucide-react"
...
export default function ProductGrid() {
  ...
  const { isSuccess, isError, isLoading, createdProduct, bulkUploadStatus } = useSelector((state) => state.product)
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      if (bulkUploadStatus) {
        toast.success(`${bulkUploadStatus.uploadedCount} Products uploaded successfully!`)
  ...
  }, [isSuccess, createdProduct, bulkUploadStatus, dispatch])
  ...
  const filteredProducts = productState
    ...
    .sort((a, b) => {
     ...
     const handleGenerateAI = async () => {
          ...
          try {
            const aiGeneratedData = await generateProductDetails(formik.values.title)
            if (aiGeneratedData) {
              formik.setValues({
                ...formik.values,
                description: aiGeneratedData.description,
                price: aiGeneratedData.price,
                brand: aiGeneratedData.brand,
                category: aiGeneratedData.category,
                tags: aiGeneratedData.tags[0],
                color: aiGeneratedData.color,
                quantity: aiGeneratedData.quantity,
              })
              setColor(aiGeneratedData.color)
            }
          } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 space-y-4 md:flex md:items-center md:space-x-4 md:space-y-0">
        ...
        <button
          ...
          className="inline-flex items-center rounded-lg border border-gray-800 px-4 py-2 text-sm font-medium text-gray-800 hover:bg-gray-100"
        >
                ...
                <div className="min-h-screen px-4 text-center">
                  <div className="inline-block w-full max-w-4xl px-6 py-12 my-8 overflow-hidden text-left align-middle bg-white rounded-lg shadow-xl">
                    ...
                    <form onSubmit={formik.handleSubmit} className="space-y-6">
                      ...
                      disabled={isGeneratingAI || !formik.values.title}
                      className="inline-flex items-center rounded-r-md border border-l-0 border-gray-300 bg-gray-50 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 focus:outline-none disabled:opacity-50"
                    >
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-50">
          <div className="min-h-screen px-4 text-center">
            <div className="inline-block w-full max-w-md px-6 py-12 my-8 overflow-hidden text-left align-middle bg-white rounded-lg shadow-xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-semibold text-gray-900">Bulk Upload Products</h3>
                <button onClick={() => setIsBulkUploadModalOpen(false)} className="rounded-full p-2 hover:bg-gray-100">
                  <X className="h-6 w-6" />
                </button>
              </div>
                  ...
                  <div className="mt-1 border-2 border-dashed border-gray-300 rounded-lg p-6">
                    ...
                    >
                      {({ getRootProps, getInputProps, isDragActive }) => (
                        <div
                          {...getRootProps()}
                          className={`text-center cursor-pointer ${isDragActive ? "bg-gray-50" : ""}`}
                        >
                          <input {...getInputProps()} />
            ...
            <div className="p-4">
              ...
              <div className="mb-4 flex flex-wrap gap-1">
                {product.color?.map((color) => (
                  <span key={color} className="rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-600">
                    {color}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-4 flex gap-2">
                <div className="flex-1 rounded-lg bg-gray-800 px-4 py-2 text-sm font-medium text-white hover:bg-black text-center">
                  <Link to={`/admin/product/${product._id}`}>
                    <button className="hover:text-white" disabled={product.quantity === 0}>
                      View Product
                    </button>
                  </Link>
                </div>
