...
const getProducts = async (data) => {
  try {
    const response = await axios.get(
      `${base_url}/product?${data?.brand ? `brand=${data.brand}&&` : ""}${data?.category ? `category=${data.category}&&` : ""}${data?.tag ? `tags=${data.tag}&&` : ""}${data?.minPrice ? `price[gte]=${data.minPrice}&&` : ""}${data?.maxPrice ? `price[lte]=${data.maxPrice}&&` : ""}${data?.sort ? `sort=${data.sort}&&` : ""}${data?.limit ? `limit=${data.limit}&&` : ""}${data?.page ? `page=${data.page}&&` : ""} `,
    )
...
const elasticSearch = async (searchData) => {
  // console.log(searchData)
    try {
      // Handle empty search
      if (!searchData?.query?.trim()) {
       return
        // return await getProducts({}) // Reuse existing getProducts function
      }
  
      const response = await axios.post(
        `${base_url}/product/search/elastic`,
        searchData,
        config,
      )
      ...
      if (error.response?.data?.details) {
        throw new Error(`Search failed: ${error.response.data.details}`)
      }
