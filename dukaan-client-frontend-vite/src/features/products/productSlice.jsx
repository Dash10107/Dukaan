...
export const elasticSearchProducts = createAsyncThunk("product/elastic-search", async (searchData, thunkAPI) => {
    try {
      return await productService.elasticSearch(searchData)
    } catch (error) {
      return thunkAPI.rejectWithValue(error.message)
    }
  })

export const getAProducts = createAsyncThunk("product/get-single", async (productId, thunkAPI) => {
  try {
...
export const productSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      builder
  .addCase(elasticSearchProducts.pending, (state) => {
    state.isSearching = true
    state.isError = false
  })
  .addCase(elasticSearchProducts.fulfilled, (state, action) => {
    state.isSearching = false
    state.isSuccess = true
    state.isError = false
    state.products = action.payload
  })
  .addCase(elasticSearchProducts.rejected, (state, action) => {
    state.isSearching = false
    state.isError = true
    state.isSuccess = false
    state.message = action.payload
